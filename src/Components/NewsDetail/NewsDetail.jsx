...
const NewsDetail = () => {
  ...
  useEffect(() => {
    const fetchNewsItem = async () => {
      try {
        const response = await fetch(`http://localhost:5000/newsDetail/${id}`);
  ...
  if (!newsItem) return <p className="p-4 text-center">Blog not found.</p>;
  ...
  return (
    <section className="p-4 sm:p-6 md:p-8 max-w-5xl w-full mx-auto">
      <div className="border p-4 sm:p-6 rounded shadow bg-white">
        ...
        <p className="text-gray-700 text-sm sm:text-base mb-4 break-words">
          {newsItem.subTitleDescription}
        </p>
        <div className="flex flex-col sm:flex-row sm:justify-between text-sm text-gray-500">
          <span>{newsItem.author}</span>
          <span>{new Date(newsItem.published).toLocaleDateString()}</span>
        </div>
      </div>
